...
<head>
    ...
    <style>
        ...
        .calendar-grid {
            display: grid;
            grid-template-columns: auto repeat(12, 40px); /* Adjusted for 12 slots */
            gap: 1px;
            min-width: 700px; 
        }
        ...
        @media (max-width: 768px) {
            ...
            .calendar-grid {
                min-width: unset;
                grid-template-columns: auto repeat(12, 1fr); /* Adjusted for 12 slots */
            }
        }
    ...
    <script>
        ...
        function saveData() {
            try {
                ...
                showToast("Erreur lors de la sauvegarde des données. L'espace de stockage local est peut-être plein.", 'error');
        ...
        function loadData() {
            try {
                const storedPersons = localStorage.getItem('personnes');
                const storedEquipes = localStorage.getItem('equipes');
                if (storedPersons) {
                    personnes = JSON.parse(storedPersons);
                    personnes.forEach(p => {
                        // Ensure new properties exist for old data and re-calculate level
                        ...
                        // Adapt old availability data to new 2-hour slots if needed
                        // This assumes old data was per hour. If it was already 2-hour, this is harmless.
                        if (typeof p.disponibilites === 'undefined' || Object.keys(p.disponibilites).length === 0 || !p.disponibilites[JOURS_SEMAINE[0]][HEURES_JOUR[0]]) {
                             p.disponibilites = generateEmptyAvailabilities(); 
                        } else {
                            // If old data exists, ensure it's compatible with new slots
                            // For simplicity, we regenerate based on new HEURES_JOUR
                            // A more complex migration would map old 1h slots to new 2h slots
                            const newDispos = generateEmptyAvailabilities();
                            JOURS_SEMAINE.forEach(day => {
                                HEURES_JOUR.forEach(newHourSlot => {
                                    // Take the first hour of the 2-hour slot to check if it was available in the old data
                                    const oldHour = parseInt(newHourSlot.split('h-')[0]);
                                    if (p.disponibilites[day] && p.disponibilites[day][`${oldHour}h`]) {
                                        newDispos[day][newHourSlot] = true;
                                    }
                                });
                            });
                            p.disponibilites = newDispos;
                        }
                ...
                if (storedEquipes) {
                    equipes = JSON.parse(storedEquipes); 
                }
            } catch (e) {
                showToast("Erreur lors du chargement des données. Les données locales pourraient être corrompues.", 'error');
        ...
        function addUpdatePerson(event) {
        ...
        function deletePerson(index) {
            ...
            if (confirm(`Êtes-vous sûr de vouloir supprimer le participant "${personName}" ?`)) {
                personnes.splice(index, 1);
                saveData();
                renderPersonnesTable();
                showToast(`Participant "${personName}" supprimé.`, 'success');
                
                const selectedCalendarIndex = parseInt(calendarPersonSelector.value);
                if (selectedCalendarIndex === index) {
                    if (personnes.length > 0) {
                        calendarPersonSelector.value = 0;
                    } else {
                        calendarPersonSelector.value = '';
                    }
                } else if (selectedCalendarIndex > index) {
                    calendarPersonSelector.value = selectedCalendarIndex - 1;
                }
                renderCalendar(); 
            }
        }
        ...
        function updateCalendarPersonSelector() {
            ...
            if (personnes.length === 0) {
                calendarPersonSelector.innerHTML = '<option value="">-- Aucune personne --</option>';
        ...
        function renderCalendar() {
            ...
            calendarGrid.dataset.personIndex = selectedIndex;
                ...
                for (const combo of teamCombinationsToTry) { 
                    if (eliteInSlot.length >= combo.elite && normalInSlot.length >= combo.normal) {
                        ...
                        if (teamResult.success) {
                            // Store how each player was counted (participation or aide)
        ...
        function resetParticipationCounters(showConfirmation = true) {
        ...
        function resetAllData() {
            if (confirm("Êtes-vous sûr de vouloir réinitialiser TOUTES les données (participants, disponibilités et équipes) ? Cette action est irréversible.")) {
                localStorage.clear();
                personnes = [];
                equipes = [];
                renderPersonnesTable();
